...
function buildLoginEnvelope({ etin, appSysId, clientSoftwareTxt = 'Toozi', cert, privateKey }) {
  ...
  // Force xmlns:ns3 onto the Timestamp element before canonicalization.
  // xml-crypto's exc-c14n only emits prefixes that are locally declared or
  // visibly utilized, but the W3C InclusiveNamespaces spec requires prefixes
  // listed in PrefixList to be emitted regardless. IRS's verifier follows the
  // spec, so their canonical form includes xmlns:ns3 (pulled from the ancestor
  // ns3:Security). Declaring it locally makes our canonicalizer emit the same
  // bytes. Only the in-memory DOM is mutated — the wire envelope is unchanged.
