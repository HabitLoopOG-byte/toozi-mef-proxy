...
function buildLoginEnvelope({ etin, appSysId, clientSoftwareTxt = 'Toozi', cert, privateKey }) {
  ...
  // RSA signature diagnostics — IRS is now returning "Signature is invalid" (all
  // three digests passed). That means the SignedInfo bytes we hash+sign differ
  // from what IRS canonicalizes from the wire envelope. Log everything we feed
  // into the signer so we can diff against the spec-expected canonical form.
  console.log('SIGNEDINFO_BUILD_TAG: signedinfo-c14n-v1');
  ...
  console.log('SIGNEDINFO_SHA256_HEX:', crypto.createHash('sha256').update(canonicalSignedInfo, 'utf8').digest('hex'));

  const signer = crypto.createSign('RSA-SHA256');
  signer.update(canonicalSignedInfo, 'utf8');
  const signatureValue = signer.sign(privateKey, 'base64');
  console.log('SIGNATURE_VALUE_LEN:', signatureValue.length);
  console.log('SIGNATURE_VALUE_FIRST_64:', signatureValue.slice(0, 64));
