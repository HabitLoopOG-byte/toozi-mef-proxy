...
// SessionKeyCd controls whether IRS issues a SAML session assertion this call
// can reuse. `Y` = create/reuse a session (required on Login). `N` = one-shot,
// don't hold a session open. For standalone signed calls (GetAcks, GetNewAcks,
// etc.) `Y` leaks sessions — IRS caps concurrent sessions per login and
// returns SOAP fault SESSNIMF0000001 "Session limit reached" once the cap is
// hit. Sessions don't auto-release until JSESSIONID times out (15 min).
function buildMeFHeaderXml({
  action,
  messageId,
  messageTs,
  etin,
  ...
  return (
    ...
    `<ETIN>${etin}</ETIN>` +
    `<SessionKeyCd>${sessionKeyCd}</SessionKeyCd>` +
...
function buildSignedGetAcksEnvelope({
  ...
  const mefHeaderXml = buildMeFHeaderXml({
    ...
    clientSoftwareTxt,
    wsuId: mefHeaderId,
    // One-shot call; don't leave a MeF session open. Prevents the
    // SESSNIMF0000001 "Session limit reached" fault we hit after the
    // debugging loop left N orphaned sessions.
    sessionKeyCd: 'N',
  });
