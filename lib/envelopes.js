...
function buildLoginEnvelope({ etin, appSysId, clientSoftwareTxt = 'Toozi', cert, privateKey }) {
  ...
  // Force xmlns:ns3 and the default xmlns onto the Timestamp element before
  // canonicalization. xml-crypto's exc-c14n only emits prefixes that are
  // locally declared or visibly utilized, but the W3C InclusiveNamespaces spec
  // requires prefixes listed in PrefixList to be emitted regardless. IRS's
  // verifier follows the spec, so their canonical form includes both
  // xmlns:ns3 (from the ancestor ns3:Security) and the default xmlns="soap-env"
  // (from the root Envelope). Declaring them locally makes our canonicalizer
  // emit the same bytes. Only the in-memory DOM is mutated — the wire
  // envelope is unchanged. (Body doesn't need this treatment because its
  // element name is unprefixed, so the default namespace is already visibly
  // utilized and xml-crypto emits it.)
  timestampEl.setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:ns3', NS.wsse);
  timestampEl.setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns', NS.soap);

  const timestampC14n = new ExclusiveCanonicalization().process(timestampEl, {
    inclusiveNamespacesPrefixList: 'ns3 #default u',
  });
