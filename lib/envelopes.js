...
function buildLoginEnvelope({ etin, appSysId, clientSoftwareTxt = 'Toozi', cert, privateKey }) {
  ...
  // false, the IDENTRUST_CERT and IDENTRUST_PRIVATE_KEY env vars are
  ...
  try {
    ...
    if (!selfVerify) {
      console.log('SIGNATURE_SELF_VERIFY_FAIL: private key does not match cert public key — check IDENTRUST_CERT vs IDENTRUST_PRIVATE_KEY');
    }
  } catch (verifyErr) {
