...
const { buildLoginEnvelope, buildSendSubmissionsEnvelope, buildGetAcknowledgementsEnvelope } = require('./envelopes');
...
const GET_ACKS_ENDPOINT_VARIANTS = {
  'direct':          `${ATS_BASE}/GetAcknowledgements`,                 // current (produces Lorem Ipsum)
  'msi_services':    `${ATS_BASE}/MSIServices/GetAcknowledgements`,     // most likely per IRS Pub 4164 R10
...
const ENDPOINTS = {
  ...
  getAcknowledgementsDefault: GET_ACKS_ENDPOINT_VARIANTS.msi_services, // new default based on diagnosis
};
 ...
 */
async function getAcknowledgements({ submissionIds, schemaVariant, endpointVariant, soapActionVariant }) {
  ...
  // Resolve the endpoint URL — named variant table lookup; default is the
  // most-likely-correct MSIServices path since the bare `/GetAcknowledgements`
  // returned Lorem Ipsum HTML on the first live test.
  const endpointKey = endpointVariant || 'msi_services';
  ...
  // SOAPAction format. IRS has used both the bare string and the fully-qualified
  // URN across MeF revisions. If the bare name produces routing errors try the
  // URN form — some gateway layers parse the URN and dispatch by it.
  ...
  console.log(
    `[mef-proxy] GET_ACKS using SAML: AssertionID=${assertionIdInUse} ` +
  ...
  const { xml: envelope, messageId } = buildGetAcknowledgementsEnvelope({
    etin,
    appSysId,
    clientSoftwareTxt: 'Toozi',
    samlAssertionXml: saml.samlAssertionXml,
  ...
  logSoap(`GET_ACKS REQUEST (messageId=${messageId}, submissionIds=${submissionIds.length}, schema=${schemaVariant || 'default'})`, envelope);
