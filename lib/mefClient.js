...
const GET_ACKS_ENDPOINT_VARIANTS = {
  // We've tested 12 combos of these paths × SOAPAction format — all return
  // IRS's 11,375-byte Akamai 404 Lorem Ipsum page, even though Login at
  // /a2a/mef/Login and SendSubmissions at /a2a/mef/mime/SendSubmissions
  // both work fine. The GetAcks URL is not discoverable by guessing; the
  // exact path has to come from Syed's WSDL. Variants here remain for
  // reference + anyone who wants to run the sweep again.
  'mime':              `${ATS_BASE}/mime/GetAcknowledgements`,            // Pub 5830 §4.3.7 (default)
  'direct':            `${ATS_BASE}/GetAcknowledgements`,                 // plain SOAP binding
  'msi_services':      `${ATS_BASE}/MSIServices/GetAcknowledgements`,
  'msi_lowercase':     `${ATS_BASE}/msiservices/GetAcknowledgements`,
  'msi_services_root': `${ATS_BASE}/MSIServices`,                         // dispatcher w/ SOAPAction routing
  'get_acks':          `${ATS_BASE}/GetAcks`,
  'transmitter':       `${ATS_BASE}/Transmitter/GetAcknowledgements`,
  // New guesses to test once — all have Akamai 404 risk:
  'mime_root':         `${ATS_BASE}/mime`,                                // MIME dispatcher at exactly /mime (routes by SOAPAction)
  'login_path':        `${ATS_BASE}/Login`,                               // reuse Login endpoint — some IRS WSDLs have one URL for all ops
...
const ENDPOINTS = {
  ...
  getAcknowledgementsDefault: GET_ACKS_ENDPOINT_VARIANTS.mime,
};
...
async function getAcknowledgements({
  ...
  // Resolve endpoint URL — named variant table lookup. Default is 'mime'
  // per IRS operator guidance (same base path as SendSubmissions).
  const endpointKey = endpointVariant || 'mime';
