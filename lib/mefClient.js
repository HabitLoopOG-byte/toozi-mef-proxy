...
const GET_ACKS_ENDPOINT_VARIANTS = {
  // THE CORRECT ENDPOINT — per IRS's prd_endpoints.properties file which
  // lists every A2A service. The operation is "GetAcks" (abbreviated), NOT
  // "GetAcknowledgements" which we had been guessing. All 15 combinations
  // of GetAcknowledgements paths × SOAPAction format returned the Akamai
  // 404 Lorem Ipsum catch-all because the operation name itself was wrong.
  'get_acks_mime':     `${ATS_BASE}/mime/GetAcks`,                        // ← DEFAULT — what IRS actually accepts
  'get_ack_mime':      `${ATS_BASE}/mime/GetAck`,                         // singular — fetches a specific ack
  'get_new_acks_mime': `${ATS_BASE}/mime/GetNewAcks`,                     // fetches un-retrieved acks

  // Legacy "GetAcknowledgements" paths kept for regression testing only —
  // all proven to return Akamai 404 Lorem Ipsum.
  'mime':              `${ATS_BASE}/mime/GetAcknowledgements`,
  'direct':            `${ATS_BASE}/GetAcknowledgements`,
  'msi_services':      `${ATS_BASE}/MSIServices/GetAcknowledgements`,
  'msi_lowercase':     `${ATS_BASE}/msiservices/GetAcknowledgements`,
  'msi_services_root': `${ATS_BASE}/MSIServices`,
  'get_acks':          `${ATS_BASE}/GetAcks`,
  'transmitter':       `${ATS_BASE}/Transmitter/GetAcknowledgements`,
  'mime_root':         `${ATS_BASE}/mime`,
  'login_path':        `${ATS_BASE}/Login`,
  'mefservices':       `${ATS_BASE}/mefservices`,
};

const ENDPOINTS = {
  login: `${ATS_BASE}/Login`,
  sendSubmissions: `${ATS_BASE}/mime/SendSubmissions`,
  getAcknowledgementsDefault: GET_ACKS_ENDPOINT_VARIANTS.get_acks_mime,
};
...
/**
 * POST a SOAP envelope wrapped as RFC 2387 multipart/related — required for
 * IRS's /a2a/mef/mime/* bindings (SendSubmissions and GetAcknowledgements).
 * GetAcks has no binary attachment to send up, but the /mime binding still
 * expects a multipart request with just Part 1 (the SOAP envelope).
 *
 * IRS returns multipart/related back with Part 1 = SOAP metadata and
 * Part 2 = a binary ZIP attachment containing per-submission ack XML(s).
 * We use arrayBuffer() instead of text() so Part 2's binary bytes survive
 * intact, and also emit base64 for transport through JSON to the caller.
 */
async function postSoapAsMime({ url, envelopeXml, soapAction, messageId }) {
  ...
  console.log(
    `[mef-proxy] MIME_WRAP boundary=${boundary} envelope=${envelopeXml.length}B total=${mimeBody.length}B`
  );
  ...
  // Binary-safe read. Previous `res.text()` mangled Part 2's ZIP bytes into
  // replacement-character garbage (the MIME boundary + XML are still ASCII
  // so they survive, but the ZIP's ack XML was unrecoverable).
  ...
  return {
    ...
    statusText: res.statusText,
    headers,
    // Text view kept for the logSoap debug print + backward compat with the
    // send-submissions flow. May contain replacement characters where
    // Part 2's binary doesn't decode as UTF-8 — that's fine for text paths.
    body: respBuf.toString('utf8'),
    // Binary-safe payload — the caller (edge function) uses this to
    // faithfully parse MIME boundaries and extract the Part 2 ZIP.
...
async function getAcknowledgements({
  ...
  // Resolve endpoint URL — named variant table lookup. Default is
  // `get_acks_mime` which points at the real IRS GetAcks operation URL.
  const endpointKey = endpointVariant || 'get_acks_mime';
  ...
  // Derive the MeF operation name from the URL path — IRS uses the
  // abbreviated GetAcks / GetAck / GetNewAcks names in both the URL and
  // the SOAPAction header (and the MeFHeader <Action> element). The
  // legacy GetAcknowledgements variants are kept for regression only;
  // they'll always 404 so picking the right operation doesn't matter for them.
  let operationName;
  if (/\/GetAcks(\b|$)/.test(endpointUrl)) operationName = 'GetAcks';
  else if (/\/GetAck(\b|$)/.test(endpointUrl)) operationName = 'GetAck';
  else if (/\/GetNewAcks(\b|$)/.test(endpointUrl)) operationName = 'GetNewAcks';
  else operationName = 'GetAcknowledgements'; // legacy path — will 404 anyway

  // The /mime/ bindings expect multipart/related transport even when there
  // is no attachment; plain /a2a/mef/* paths expect text/xml. Flag it here.
  const useMimeTransport = /\/mime\//.test(endpointUrl);

  // SOAPAction header format variants — derived from operationName so they
  // track the endpoint. For the real `GetAcks` URL the SOAPAction is
  // `GetAcks`; for legacy `GetAcknowledgements` URLs it stays the old name
  // (which won't matter because those URLs 404 either way).
  const soapActionsByKey = {
    'bare':       operationName,
    'quoted':     `"${operationName}"`,
    'urn':        `http://www.irs.gov/a2a/mef/${operationName}`,
    'quoted_urn': `"http://www.irs.gov/a2a/mef/${operationName}"`,
  };
  ...
  // Pick the envelope builder based on the operation name. GetAcks has its
  // own builder (different MeFHeader <Action>, different request wrapper);
  // GetAcknowledgements falls through to the legacy builder.
  const usesGetAcksBuilder = operationName === 'GetAcks';

  if (mode === 'signed') {
    console.log(
      `[mef-proxy] GET_ACKS mode=signed (X.509) op=${operationName} ` +
      `submissionIds=${submissionIds.length} schema=${schemaVariant || 'default'} ` +
      `endpoint=${endpointKey} soapAction=${soapActionKey}`
    );
    if (usesGetAcksBuilder) {
      ({ xml: envelope, messageId } = buildSignedGetAcksEnvelope({
        etin, appSysId, clientSoftwareTxt: 'Toozi',
        cert, privateKey, submissionIds, schemaVariant,
      }));
    } else {
  ...
  console.log(`[mef-proxy] GET_ACKS_TRANSPORT=${useMimeTransport ? 'multipart/related' : 'text/xml'}`);
  logSoap(
  ...
  if (useMimeTransport) {
    ...
    response = await postSoap(endpointUrl, envelope, soapAction);
  }

  logSoap(
    `GET_ACKS RESPONSE status=${response.status} ${response.statusText || ''} (${response.durationMs}ms) bodyBytes=${response.body.length} contentType=${response.headers['content-type'] || '(none)'}`,
    response.body
  );
  ...
  return {
    ...
    soapAction,
    transport: useMimeTransport ? 'multipart/related' : 'text/xml',
    request: envelope,
    response,
  };
}
