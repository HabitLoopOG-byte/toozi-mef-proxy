...
// other.
if (new URL(ENDPOINTS.login).host !== ATS_HOST || new URL(ENDPOINTS.sendSubmissions).host !== ATS_HOST) {
  throw new Error(
    `mefClient endpoint drift: login=${new URL(ENDPOINTS.login).host}, sendSubmissions=${new URL(ENDPOINTS.sendSubmissions).host}, expected=${ATS_HOST}`
  );
}
