...
const ENDPOINTS = {
  ...
  getAcknowledgements: `${ATS_BASE}/GetAcknowledgements`,
};
// Sanity check at module load — loud failure if someone edits one without the
// other.
if (
  new URL(ENDPOINTS.login).host !== ATS_HOST ||
  new URL(ENDPOINTS.sendSubmissions).host !== ATS_HOST ||
  new URL(ENDPOINTS.getAcknowledgements).host !== ATS_HOST
) {
  throw new Error(
    `mefClient endpoint drift: login=${new URL(ENDPOINTS.login).host}, sendSubmissions=${new URL(ENDPOINTS.sendSubmissions).host}, getAcks=${new URL(ENDPOINTS.getAcknowledgements).host}, expected=${ATS_HOST}`
  );
}
 ...
 */
async function getAcknowledgements({ submissionIds, schemaVariant }) {
  ...
  console.log(
    ...
    `(ensureSamlAssertion took ${ensureDurationMs}ms) submissionIds=${submissionIds.length} schema=${schemaVariant || 'default'}`
  );
  ...
  console.log(`[mef-proxy] GET_ACKS_URL=${ENDPOINTS.getAcknowledgements}`);
  logSoap(`GET_ACKS REQUEST (messageId=${messageId}, submissionIds=${submissionIds.length}, schema=${schemaVariant || 'default'})`, envelope);

  const response = await postSoap(ENDPOINTS.getAcknowledgements, envelope, 'GetAcknowledgements');

  logSoap(
    `GET_ACKS RESPONSE status=${response.status} (${response.durationMs}ms)`,
    response.body
  );
